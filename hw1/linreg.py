...
class LinearRegression:
    ...
    def gradientDescent(self, X, y, theta):
        ...
        for i in xrange(self.n_iter):
            ...
            new_theta = np.matrix(np.zeros_like(self.theta))

            for j in range(d):
                ratio = 0
                for k in range(n): 
                    ratio += (np.dot(X[k],theta)[0,0]-y[k])*X[k,j]                
                new_theta[j,0] = theta[j,0]-self.alpha*np.divide(1,float(n))*ratio
    ...
    def computeCost(self, X, y, theta):
        ...
        n,d = X.shape
        return np.divide(1,2*float(n))*np.dot(np.transpose(np.matmul(X,theta)-y),np.matmul(X,theta)-y)[0,0]
